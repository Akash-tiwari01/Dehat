<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>DeHaat UI Clone</title>
  <!-- Bootstrap CSS -->
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
  <!-- Bootstrap Icons -->
  <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.10.5/font/bootstrap-icons.css">
  <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.10.5/font/bootstrap-icons.css">
  <link rel="stylesheet" href="assets/css/styles.css">
  <link rel="stylesheet" href="assets/css/about.css">
  <link href="https://fonts.googleapis.com/css2?family=Bitter:wght@700&family=Lato&display=swap" rel="stylesheet">
</head>
<body>

<main class="main_wrapper">
    <?php include 'header2.php'?>
        <section class="aboutarea pt-4 pb-4">
            <div class="container">
                <div class="row">
                    <div class="col-xl-6 col-lg-6">
                        <div class="aboutarea_5_img">
                            <img loading="lazy" src="assets/images/about-us/Asset1.png" alt="KSHITIJ CodeForge founders mentoring students in campus">
                        </div>
                    </div>

                    <div class="col-xl-6 col-lg-6">
                        <div class="aboutarea_content">
                            <h3 class="heading_xl">About Us</h3>
                            <p class="text_wrapper"><strong>One Click Agri Science Pvt Ltd</strong> is an innovative agricultural startup revolutionizing farming practices by providing farmers with efficient, eco-friendly solutions to enhance crop yield and quality. We are at the forefront of sustainable agriculture, combining technology with agri-science to deliver effective solutions.</p>
                            <p class="text_wrapper">Currently, we specialize in precision pesticide products that minimize environmental impact while maximizing effectiveness. Our solutions incorporate smart application technologies to target specific areas, reducing chemical usage while maintaining crop protection standards.</p>
                            <p class="text_wrapper">Our team comprises agri-scientists and technologists committed to developing products that offer convenient, quality protection against damaging insects. With formulations containing robust rates of active ingredients, we provide long residual control while adhering to sustainable agricultural practices. Our vision aligns with the global shift toward precision farming and digital solutions that improve both productivity and environmental sustainability in agriculture.</p>
                        </div>
                    </div>
                </div>
            </div>
        </section>

        <section class="aboutarea pt-4 pb-4">
            <div class="container">
                <h3 class="heading_xl text-center text-capitalize">Our Values</h3>
                <div class="heading_md text-center text-capitalize">Strengthening Our Roots | Nurturing Our Drive | Harvesting Our True Purpose</div>
                <div class="row">
                    <div class="col-xl-3 col-lg-3 col-md-6 p-5">
                        <div class="aboutarea_card">
                            <div class="aboutarea_card_img">
                                <img src="assets/images/icons/t.webp" alt="">
                            </div>
                            <h1 class="aboutarea_card_title">Entrepreneurship & Focus</h1>
                            <p class="aboutarea_card_discription">We approach our work with ownership and accountability, setting ambitious goals and seeking innovative solutions. Our decisive actions drive agricultural transformation and sustainable growth.</p>
                        </div>
                    </div>
                    <div class="col-xl-3 col-lg-3 col-md-6 p-5">
                        <div class="aboutarea_card">
                            <div class="aboutarea_card_img">
                                <img src="assets/images/icons/agricultureInovation.png" alt="">
                            </div>
                            <h1 class="aboutarea_card_title">Integrity & Responsibility</h1>
                            <p class="aboutarea_card_discription">We uphold the highest ethical standards in all operations. As stewards of the environment, we prioritize the health and safety of our employees, communities, and the farming ecosystem.</p>
                        </div>
                    </div>
                    <div class="col-xl-3 col-lg-3 col-md-6 p-5">
                        <div class="aboutarea_card">
                            <div class="aboutarea_card_img">
                                <img src="assets/images/icons/agricultureInovation.png" alt="">
                            </div>
                            <h1 class="aboutarea_card_title">Efficiency & Competitiveness</h1>
                            <p class="aboutarea_card_discription">We deliver compelling value through quality products and cost-efficient solutions. Continuous improvement drives our processes, ensuring sustainable agricultural advancements.</p>
                        </div>
                    </div>
                    <div class="col-xl-3 col-lg-3 col-md-6 p-5">
                        <div class="aboutarea_card">
                            <div class="aboutarea_card_img">
                                <img src="assets/images/icons/agricultureInovation.png" alt="">
                            </div>
                            <h1 class="aboutarea_card_title">Persistence & Teamwork</h1>
                            <p class="aboutarea_card_discription">We combine diverse talents to achieve common goals, overcoming challenges through collaboration. Effective communication, conflict resolution, and mutual respect define our work culture.</p>
                        </div>
                    </div>
                </div>
            </div>
        </section>

        <section class="aboutarea pb-4 pt-4">
            <div class="container">
                <div class="row mb-4">
                    <div class=" d-flex justify-content-center gap-3">
                        <div class="about_area_nav_buttons">
                        <button class="aboutarea-nav-btn active" data-target="tab1">Home</button>
                        <div class="divider"></div>
                        <button class="aboutarea-nav-btn" data-target="tab2">Profile</button>
                        <div class="divider"></div>
                        <button class="aboutarea-nav-btn" data-target="tab3">Contact</button>
                        </div>
                    </div>
                </div>

                <!-- Row for Tab Contents -->
                <div class="row">
                <div class="col">
                    <div class="tab-content container">
                    <div id="tab1" class="tab-pane active">
                        <div class="container">
                            <div class="row">
                                <div class="col-xl-6 col-lg-6 col-md-12 d-flex align-items-center">
                                    <div class="aboutarea_tab_content ">
                                        <ul>
                                            <li>
                                                <h3 class="text_wrapper_list_title">Mutual Respect</h3>
                                                <p class="text_wrapper_list_item">We believe in the power of mutual respect to maintain an honest, transparent and meaningful relationship with our stakeholders.</p>
                                            </li>
                                            <li>
                                                <h3 class="text_wrapper_list_title">Practice Integrity, Drive Accountability</h3>
                                                <p class="text_wrapper_list_item">We practice integrity, drive accountability and ownership to achieve our common goals and maximize employee potential.</p>
                                            </li>
                                            <li>
                                                <h3 class="text_wrapper_list_title">Diverse, Equitable & Inclusive Culture</h3>
                                                <p class="text_wrapper_list_item">We embrace diversity across cultural background, race, religion, caste, gender, opinions, & views as the core of our culture</p>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                                <div class="col-xl-6 col-lg-6 col-md-12">
                                    <div class="aboutarea_tab_image">
                                        <img src="assets/images/about-us/respect.jpg" alt="">
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div id="tab2" class="tab-pane">
                        <div class="container">
                            <div class="row">
                                <div class="col-xl-6 col-lg-6 col-md-12 d-flex align-items-center">
                                    <div class="aboutarea_tab_content ">
                                        <ul>
                                            <li>
                                                <h3 class="text_wrapper_list_title">Mutual Respect</h3>
                                                <p class="text_wrapper_list_item">We believe in the power of mutual respect to maintain an honest, transparent and meaningful relationship with our stakeholders.</p>
                                            </li>
                                            <li>
                                                <h3 class="text_wrapper_list_title">Practice Integrity, Drive Accountability</h3>
                                                <p class="text_wrapper_list_item">We practice integrity, drive accountability and ownership to achieve our common goals and maximize employee potential.</p>
                                            </li>
                                            <li>
                                                <h3 class="text_wrapper_list_title">Diverse, Equitable & Inclusive Culture</h3>
                                                <p class="text_wrapper_list_item">We embrace diversity across cultural background, race, religion, caste, gender, opinions, & views as the core of our culture</p>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                                <div class="col-xl-6 col-lg-6 col-md-12">
                                    <div class="aboutarea_tab_image">
                                        <img src="assets/images/about-us/innovation-and-creativity.jpg" alt="">
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div id="tab3" class="tab-pane">
                        <div class="container">
                            <div class="row">
                                <div class="col-xl-6 col-lg-6 col-md-12 d-flex align-items-center">
                                    <div class="aboutarea_tab_content ">
                                        <ul>
                                            <li>
                                                <h3 class="text_wrapper_list_title">Mutual Respect</h3>
                                                <p class="text_wrapper_list_item">We believe in the power of mutual respect to maintain an honest, transparent and meaningful relationship with our stakeholders.</p>
                                            </li>
                                            <li>
                                                <h3 class="text_wrapper_list_title">Practice Integrity, Drive Accountability</h3>
                                                <p class="text_wrapper_list_item">We practice integrity, drive accountability and ownership to achieve our common goals and maximize employee potential.</p>
                                            </li>
                                            <li>
                                                <h3 class="text_wrapper_list_title">Diverse, Equitable & Inclusive Culture</h3>
                                                <p class="text_wrapper_list_item">We embrace diversity across cultural background, race, religion, caste, gender, opinions, & views as the core of our culture</p>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                                <div class="col-xl-6 col-lg-6 col-md-12">
                                    <div class="aboutarea_tab_image">
                                        <img src="assets/images/about-us/farmer-first.jpg" alt="">
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    </div>
                </div>
                </div>
            </div>
        </section>




    <?php include 'footer.php'?>
    <script src="assets/js/aboutus.js"></script>

</body>
</html>